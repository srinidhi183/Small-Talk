...
# -------- Excel safe saver --------
def safe_to_excel(df: pd.DataFrame, base_name="search_bar_noopt_checklist.xlsx", out_dir="reports", engine="xlsxwriter") -> str:
    ...
    for i in range(0, 100):
        p = out_dir / f"{stem}_{ts}{'' if i==0 else f'_{i}'}{suffix}"
        try:
            df.to_excel(p, index=False, engine=engine)
            print(f"Checklist written to: {p}")
            return str(p)
        except PermissionError:
            continue
...
# -------- Type A Acceptance Checklist builder --------
def build_checklist_rows_type_a(output_obj: Dict[str, Any], run_note: str, raw_json_path: Path) -> List[Dict[str, str]]:
    ...
    # 1) Run & Repro
    rows.append({"Checklist Item":"Ran: search_bar('<keywords>', allow_options=False)","Result":"Executed","Present Value (from output)":run_note,"Pass/Fail":"Pass"})
    ...
    # 2) JSON Validity & Keys
    is_obj = isinstance(output_obj, dict)
    add("Output is valid JSON", "Checked", type(output_obj).__name__, is_obj)
    add("Single JSON object (not list/string dump)", "Checked", type(output_obj).__name__, is_obj and not isinstance(output_obj, list))

    only_keys_ok, key_diff = _has_only_keys(output_obj if is_obj else {}, TYPE_A_TOP_KEYS) if is_obj else (False, [])
    add("All required top-level keys present", "Checked", {"diff": key_diff}, only_keys_ok)

    nested_paths = [
        "category.categorie_name","subcatetgory.subcatetgory_name","topic.name","topic.topic_tags.tag_names",
        "general_facts.general_definition","general_facts.general_points","general_facts.general_fun_fact","general_facts.key_facts_text","general_facts.key_facts_fun_fact",
        "news.news_text","news.news_fun_facts",
        "interesting_trivia.trivia_text","interesting_trivia.trivia_fun_fact",
        "opinions.opinions_text","opinions.opinions_fun_fact",
        "questions.questions_text","questions.questions_fun_fact",
    ]
    ...
    add("category.categorie_name is non-empty string", "Checked", _get(output_obj,"category.categorie_name",""), bool(str(_get(output_obj,"category.categorie_name","")).strip()))
    add("subcatetgory.subcatetgory_name is non-empty string", "Checked", _get(output_obj,"subcatetgory.subcatetgory_name",""), bool(str(_get(output_obj,"subcatetgory.subcatetgory_name","")).strip()))
    add("topic.name is non-empty string", "Checked", _get(output_obj,"topic.name",""), bool(str(_get(output_obj,"topic.name","")).strip()))
    ...
    gf_fun = _get(output_obj,"general_facts.general_fun_fact","")
    add("general_facts.general_fun_fact is one fun fact", "Checked", gf_fun, isinstance(gf_fun,str) and 1 <= _sentence_count(gf_fun) <= 2 and _word_count(gf_fun) >= 4)
    ...
    kf_fun = _get(output_obj,"general_facts.key_facts_fun_fact","")
    add("general_facts.key_facts_fun_fact is one fun fact", "Checked", kf_fun, isinstance(kf_fun,str) and 1 <= _sentence_count(kf_fun) <= 2 and _word_count(kf_fun) >= 4)
    ...
    add("news.news_text has 3 bullet-style sentences tied to general info", "Checked", news_text, ncount >= 3)

    news_fun = _get(output_obj,"news.news_fun_facts","")
    add("news.news_fun_facts is one fun fact", "Checked", news_fun, isinstance(news_fun,str) and 1 <= _sentence_count(news_fun) <= 2 and _word_count(news_fun) >= 4)
...
# -------- main runner (no options) --------
def run_search_bar(keywords, allow_options=False, prompt_version="v01",
    ...
    for attempt in range(1, MAX_RERUNS + 1):
        ...
        raw_json_file = out_dir / f"search_bar_noopt_raw_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        raw_json_file.write_text(json.dumps(json_output, ensure_ascii=False, indent=2), encoding="utf-8")

        run_note = f"search_bar(keywords='{keywords}', allow_options=False, prompt_version='{prompt_version}')"
        ...
        safe_to_excel(df, base_name="search_bar_noopt_checklist.xlsx")
        # also save the human-readable JSON requested name
        with open(out_dir / f"search_bar_no_opt_output_{keywords.replace(' ','_')}.json", "w", encoding="utf-8") as f:
    ...
    if last_candidate is not None:
        ...
        out_dir = Path("reports"); out_dir.mkdir(parents=True, exist_ok=True)
        raw_json_file = out_dir / f"search_bar_noopt_raw_FAILED_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        raw_json_file.write_text(json.dumps(last_candidate, ensure_ascii=False, indent=2), encoding="utf-8")

        run_note = f"[FAILED mode] search_bar(keywords='{keywords}', allow_options=False, prompt_version='{prompt_version}')"
        rows = build_checklist_rows_type_a(last_candidate, run_note, raw_json_file)
        df = pd.DataFrame(rows, columns=["Checklist Item","Result","Present Value (from output)","Pass/Fail"])
        safe_to_excel(df, base_name="search_bar_noopt_checklist_FAILED.xlsx")
        with open(out_dir / f"search_bar_no_opt_output_{keywords.replace(' ','_')}_FAILED.json", "w", encoding="utf-8") as f:
            json.dump(last_candidate, f, ensure_ascii=False, indent=2)
    return None

# Example usage
if __name__ == "__main__":
    keywords = "Market Capitilisation"
